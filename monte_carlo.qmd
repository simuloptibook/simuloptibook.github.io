...
well happen because PRNG might accidentally "cluster" samples in a specific region. Alas, that would 
...
Until now, all methods for standard Monte Carlo that we have seen so far rely explicitly on the assumption that we can generate independent samples directly from the target distribution. 
...
In the next sections, we will explore the foundations of MCMC, including the **Metropolis-Hastings** algorithm as one of the most widely used MCMC techniques.
