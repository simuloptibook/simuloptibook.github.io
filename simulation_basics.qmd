...
$$
\begin{aligned}
S_1 & = \{X_1,X_2,X_3,\dots,X_{n-1}\} \\
S_2 & = \{X_2,X_3,X_4,\dots,X_n\}
\end{aligned}
$$
...
is widely used in practice. We start by generating two uniform random numbers $u_1, u_2 \sim U(0,1)$.

* Calculate $R=\sqrt{-2\ln(u_1)}$.
* Calculate $\theta=2\pi u_2$.
* Calculate $z_1=R\cos(\theta)$ and $z_2=R\sin(\theta)$.
* Then $z_1$ and $z_2$ are independent standard normal random variables, i.e., $z_1, z_2 \sim N(0,1)$.

Core idea: we are transforming the uniform random variables into polar coordinates, where $R$ is the radius and $\theta$ is the angle. The transformation ensures that the resulting $z_1$ and $z_2$ 
...
We now switch from *discrete* to *continuous* time. The **Poisson process** is a canonical 
